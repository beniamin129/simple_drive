...
class DatabaseStorageAdapterTest < ActiveSupport::TestCase
  setup do
    @adapter = DatabaseStorageAdapter.new
  end

  test "store saves data to database" do
    ...
    assert result
    assert BlobStorage.exists?(id: blob_id)
  end

  test "retrieve gets data from database" do
    blob_id = "test-#{Time.now.to_i}"
    ...
    assert_equal base64_data, retrieved_data
  end
  ...
  test "delete removes data from database" do
    blob_id = "test-#{Time.now.to_i}"
    ...
    assert result
    assert_not BlobStorage.exists?(id: blob_id)
  end

  test "exists? checks if blob exists in database" do
    blob_id = "test-#{Time.now.to_i}"
    base64_data = Base64.encode64("Test data")
  ...
  test "size returns data size" do
    blob_id = "test-#{Time.now.to_i}"
